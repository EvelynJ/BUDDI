...
;if fitting bulge+disk, fix disc sersic index as 1, otherwise, allow it some variation)
n_poly=comp3_poly[2]
comp3_re_polynomial_in=comp3_poly[0]
comp3_mag_polynomial_in=comp3_poly[1]
...
;        psf+=',psf_'+string(n,format='(I4.4)')+'.fits'
        ...
        sky+=','+string(res.SKY_GALFIT_BAND,format='(F08.4)')
        sky_grad+=',0.0'
        x_D+=','+string(res.X_GALFIT_D,format='(F05.2)')
        y_D+=','+string(res.Y_GALFIT_D,format='(F05.2)')
        mag_D+=','+string(res.MAG_GALFIT_BAND_D,format='(F05.2)')
        Re_D+=','+string(res.RE_GALFIT_BAND_D,format='(F06.2)')
        n_D+=','+string(res.N_GALFIT_BAND_D,format='(F05.2)')
        q_D+=','+string(res.Q_GALFIT_D,format='(F04.2)')
        pa_D+=','+string(res.PA_GALFIT_D,format='(F06.2)')
        
        ;insert parameters for bulge
        if n_comp ge 1100 then begin
          x_B+=','+string(res.X_GALFIT_B,format='(F05.2)')
          y_B+=','+string(res.Y_GALFIT_B,format='(F05.2)')
          mag_B+=','+string(res.MAG_GALFIT_BAND_B,format='(F05.2)')
          Re_B+=','+string(res.RE_GALFIT_BAND_B,format='(F06.2)')
          n_B+=','+string(res.N_GALFIT_BAND_B,format='(F05.2)')
          q_B+=','+string(res.Q_GALFIT_B,format='(F04.2)')
          pa_B+=','+string(res.PA_GALFIT_B,format='(F06.2)')
        endif
