...
;##############################################################
;##############################################################
pro BUDDI,input_file,KEEP_CUBES=keep_cubes,GC=gc,AUTO=auto
...
;  window, xsize=r[0]*0.4, ysize=r[1]*0.8
 ...
 ;signal/10 for victors datacubes
     voronoi_2d_binning, xpix, ypix, signal, noise, targetSN, $
                         binNum, xnde, ynde, xBar, yBar, sn, nPixels, scale, root,galaxy_ref, /QUIET
     
     while keyword_set(auto) and n_elements(sn) lt 2 do begin
        targetSN=targetSN/2
        voronoi_2d_binning, xpix, ypix, signal, noise, targetSN, $
                            binNum, xnde, ynde, xBar, yBar, sn, nPixels, scale, root,galaxy_ref, /QUIET
     endwhile
...
;Start by measuring the kinematics of the central bin
  ...
  if setup.measure_kinematics eq 'y' then begin
     print,'############################'
     print,'# Measuring the kinematics #'
     print,'############################'
...
;  sample = where(wavelength gt alog10(6500) and wavelength lt alog10(8000))
     sample = where(wavelength gt alog10(4000) and wavelength lt alog10(6500))
;  sample = where(wavelength gt alog10(5200) and wavelength lt alog10(8700))
...
;    goodpixels=goodPixels1[suba]
        if setup.MUSE_MODE eq 'No' or setup.MUSE_MODE eq 'NoAO' then goodPixels = indgen(n_elements(galaxy)) $
        else goodPixels = where(wave lt 5749 or wave gt 6013 ) ;mask out the notch region ij extended mode
        
; Here the actual fit starts. The best fit is plotted on the screen.
; Gas emission lines are excluded from the pPXF fit using the GOODPIXELS keyword. Nope!
;
...
;identify any outlying points, and remove from fit
 ;outliers include those values where sigma=sigma_in or sigma_max (=1000)
     ...
     m=0
     l=0
     if n_elements(kinematics_temp2[1,*]) gt 2 then begin
        for n=0,new_central_bin[0],1 do begin
           if n gt new_central_bin[0]-3 then offset=-3 else offset=3
           if n_elements(kinematics_temp2[1,*]) lt 3 then offset=1
           s=size(kinematics_temp2)
 ;if s[2] le offset then offset=s[2]-1
        ...
        for n2=new_central_bin[0]+1,n_bins-1,1 do begin
           s=size(kinematics_temp2)
           if s[2] lt 10 then offset=s[2]-2 else offset=10
           if (kinematics_temp2[1,n2]-kinematics_temp2[1,n2-offset]) lt 300 then begin 
              kinematics_temp3[*,m]=kinematics_temp2[*,n2]
              if gas eq 'y' then kinematics_gas3[*,m]=kinematics_gas2[*,n2]
              m+=1
           endif 
           if (kinematics_temp2[1,n2]-kinematics_temp2[1,n2-offset]) ge 300 then begin
...
;  identify binned emission spectrum for each spaxel, and subtract
     ...
     if n_elements(central_bin) gt 1 then central_bin=central_bin[0]
...
;  sigma0=60.
     ...
     for ll=0,n_elements(bin_kinematics)-1,1 do begin
 ;calculate velocity corrections
        wavelengthshift_linear=(velocity_bin[ll]-vel0)/3e5*central_wavelength
        wavelengthshift_log=alog10(wavelengthshift_linear+central_wavelength)-alog10(central_wavelength)
        vel_correction[ll]=wavelengthshift_log/step ;wavelength correction necessary for each bin
        if vel_correction[ll] ge 0 then vel_correction[ll]+=0.5 else vel_correction[ll]-=0.5
        vel_correction[ll]=long(vel_correction[ll])
        
 ;calculate sigma corrections
        if sigma_bin[ll] le sigma0 then FWHM_dif = SQRT((2.355*sigma0/velScale)^2 - (2.355*sigma_bin[ll]/velScale)^2)  $ ;divide sigma by velScale to get value in pixels
        else FWHM_dif=0.
        sigma_correction[ll] = FWHM_dif/2.355 ;*step ; Sigma difference in pixels 
     endfor
...
;        corrected_IFU[x_final[m]+x_centre,y_final[m]+y_centre,*]=gaussfold(wavelength,intermediate_IFU[x_final[m]+x_centre,y_final[m]+y_centre,*],sigma_correction[bins_final[m]]) $
        ...
        endif else if signal[m]/noise[m] lt 5 then begin ;and signal[m]/noise[m] gt 0 then begin
           x_final[m]=xpix_total[m]
           y_final[m]=ypix_total[m]
...
;      temparray[1,m]=y_final[m]+y_centre
;      temparray[2,m]=0
;      intermediate_IFU[x_final[m]+x_centre,y_final[m]+y_centre,*]=input_IFU[x_final[m]+x_centre,y_final[m]+y_centre,*]
;      corrected_IFU[x_final[m]+x_centre,y_final[m]+y_centre,*]=intermediate_IFU[x_final[m]+x_centre,y_final[m]+y_centre,*]
;    endelse
     endfor
...
;3b. Prepare galfitm readme file and run galfitm with all free parameters
  ...
  if n_comp eq 1000 then imgblock='imgblock_single.fits' else imgblock='imgblock_double.fits' 
  if file_test(root+decomp+median_dir+imgblock) eq 1 then begin
     nband=1
     if setup.disk_type eq 'Sersic' then setup.disk_type='sersic'
     if setup.bulge_type eq 'Sersic' then setup.bulge_type='sersic'
     if setup.comp3_type eq 'Sersic' then setup.comp3_type='sersic'
     ...
     if setup.comp3_type eq 'PSF' then setup.comp3_type='psf'
     if setup.comp4_type eq 'PSF' then setup.comp4_type='psf'
     if n_comp eq 1000 then res=read_sersic_results_2comp(root+decomp+median_dir+imgblock, nband, bd=0) $
     else if n_comp ge 1100  and setup.bulge_type eq 'sersic' then res=read_sersic_results_2comp(output+median_dir+imgblock, nband, bd=1) $
     else  res=read_sersic_results_2comp(output+median_dir+imgblock, nband, bd=0)
     
     fits_read,root+decomp+median_dir+'image.fits',image_temp,h_temp
     s=size(image_temp)
     if n_comp eq 1000 then max_Re=max(res.RE_GALFIT_BAND_D)
     if n_comp ge 1100 and setup.bulge_type eq 'sersic' then max_Re=max([res.RE_GALFIT_BAND_D,res.RE_GALFIT_BAND_B])
     if n_comp ge 1100 and setup.bulge_type eq 'psf' then max_Re=max(res.RE_GALFIT_BAND_D)
     
     
     if file_test(root+decomp+median_dir+imgblock) eq 0 or max_Re gt s[1]*3 then begin
        setup.decompose_binned_images='n'
        setup.decompose_image_slices='n'
        setup.create_subcomps='n'
        setup.visualise_results='n'
        print,'Unreasonable parameters measured for the median fit, terminating BUDDI here.'
     endif
  endif else begin
     setup.decompose_binned_images='n'
     setup.decompose_image_slices='n'
     setup.create_subcomps='n'
     setup.visualise_results='n'
     print,'Fit to the median image failed, terminating BUDDI here.'
 ...
 ;results from median fit or free fit as starting parameters
           ...
           if ~keyword_set(auto) then begin
              ...
              if rep eq 2 then decision='b'
              if rep eq 3 then decision='q'
           endelse
...
;       else if n_comp eq 1110 and setup.comp3_type eq 'psf' then multiplot
 ...
 ;user inpiut to determine if polynomials are ok
           ...
           endif else if keyword_set(auto) and rep eq 1 then decision='n' $
           else if keyword_set(auto) and rep eq 2 then decision='y'
...
;######################################
;Put a catch in case of unrealistic values after fitting the 
;binned images. This will prevent the code from crashing when 
;running in automatic mode
  imgblock='imgblock.fits'
  if file_test(root+decomp+binned_dir+imgblock) eq 1 then begin
     if setup.disk_type eq 'Sersic' then setup.disk_type='sersic'
     if setup.bulge_type eq 'Sersic' then setup.bulge_type='sersic'
     if setup.comp3_type eq 'Sersic' then setup.comp3_type='sersic'
     if setup.comp4_type eq 'Sersic' then setup.comp4_type='sersic'
     ...
     if setup.comp4_type eq 'PSF' then setup.comp4_type='psf'
     if n_comp eq 1000 then res=read_sersic_results_2comp(root+decomp+binned_dir+'imgblock.fits', nband, bd=0) $
     else if n_comp ge 1100  and setup.bulge_type eq 'sersic' then res=read_sersic_results_2comp(output+binned_dir+'imgblock.fits', nband, bd=1)
     ...
     if max_Re gt s[1]*3 then begin
        setup.decompose_image_slices='n'
        setup.create_subcomps='n'
        setup.visualise_results='n'
        print,'Unreasonable parameters measured for the binned fit, terminating BUDDI here.'
     endif
...
;6. create bulge and disc data cubes
  if setup.visualise_results eq 'y' and file_test(root+decomp+slices_dir+'subcomps*') eq 1 then begin
     print,'###################'
...
;  if keyword_set(~keep_cubes) then spawn,'rm '+root+decomp+decomp_dir+'component1_flux.fits'
  ...
  endif else if setup.visualise_results eq 'y' and file_test(root+decomp+slices_dir+'subcomps*') eq 0 then $
     print,'Final products not created- no subcomps files'
