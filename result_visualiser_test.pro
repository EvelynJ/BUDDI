...
pro result_visualiser_test,root,directory,decomp,galaxy_ref,slices_dir,info,x_centre,y_centre,start_wavelength,end_wavelength,wavelength,Redshift,n_comp,comp3_type,comp4_type,comp4_x,comp4_y,no_slices,MANGA=manga,CALIFA=califa
first_image=info[0]
final_image=info[1] 
no_bins=info[2]
...
;temp=where(wavelength_in ge start_wavelength-0.5 and wavelength_in le end_wavelength)
wavelength=fltarr(npix)
;wavelength=wavelength_in[temp]
;ss=where(wavelength ge start_wavelength)
step=sxpar(h_bulge,'CD3_3')
wavelength0=alog10(start_wavelength);sxpar(h_temp,'CRVAL3');+ss*step
for n=0,npix-1,1 do wavelength[n]=10^(wavelength0+n*step)
...
n_binned=n_elements(res2.PA_GALFIT_BAND_D)
PA=res2.PA_GALFIT_BAND_D[0:n_binned-1]
no_slices=info[2]
...
end_wavelength=7000;10000;7000
...
for bd=1,xx,1 do begin
    ...
    temp=sxpar(h,'CRVAL3')
    sxaddpar,h,'CRVAL1',alog10(wavelength[0]);temp
    temp=sxpar(h,'CD3_3')
    sxaddpar,h,'CD1_1',alog10(wavelength[1])-alog10(wavelength[0]);temp
...
!p.multi=0;[0,1,4]
;start_wavelength=4600
end_wavelength=7200;10300;6900
