...
pro bin_datacube_test, datacube, no_bins, root,dir,decomp,binned_dir,slices_dir,median_dir,$
...
if keyword_set(galaxy) then begin
  fits_read,directory+decomp+galaxy_ref+'_smoothed_kinematics.fits', spec_in, header1;,exten_no=0
...
;  wavelength0=sxpar(header1,'CRVAL3')
  ...
  fits_read,directory+file+'.fits',temp_input,h,exten_no=0
...
;    wavelength_selected
  ...
  fits_read,directory+decomp+slices_dir+name+string(first_image,format='(i4.4)')+'.fits',temp_input,h,exten_no=0
;  h = headfits(directory+decomp+slices_dir+name+string(first_image,format='(i4.4)')+'.fits')
  ...
  for run=0,no_bins,1 do begin
      ...
      fits_read,directory+decomp+slices_dir+name+string(0.5*(a+b),format='(i4.4)')+'.fits',crap,h,exten_no=0
;      h = headfits(directory+decomp+slices_dir+name+string(0.5*(a+b),format='(i4.4)')+'.fits')
  ...
  fits_read,directory+decomp+slices_dir+name+string(final_image,format='(i4.4)')+'.fits',crap,h,exten_no=0
