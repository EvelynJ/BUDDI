PRO srccor,x1in,y1in,x2in,y2in,dcr,ind1,ind2,option=option,magnitude=magnitude,$
  ...
  IF NOT keyword_set(silent) THEN message,/info,'Option code = '+strtrim(option,2)
  ...
  ;
  n1=n_elements(x1)
  IF NOT keyword_set(silent) THEN message,/info,strtrim(n1,2)+' sources in list 1'
  n2=n_elements(x2)
  IF NOT keyword_set(silent) THEN message,/info,strtrim(n2,2)+' sources in list 2'
  nmch=0L
  ind1=-1L & ind2=-1L
  ...
  IF NOT keyword_set(silent) THEN message,/info,strtrim(n_elements(ind1),2)+' left in list 1'
  IF NOT keyword_set(silent) THEN message,/info,strtrim(n_elements(ind2),2)+' left in list 2'
