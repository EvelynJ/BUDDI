...
pro result_visualiser_GC,setup,info,start_wavelength,end_wavelength,wavelength,$
  original_datacube,bestfit_datacube,residual_datacube,disk_datacube,$
  residual_sky_datacube,bulge_datacube,comp3_datacube,MANGA=manga,CALIFA=califa,GC=gc
...
if n_comp eq 1111 then al_legend,['Integrated spectrum from datacube','Centre + Bulge + Disc + comp4','Bulge','Centre','Disc','Comp4','Residuals'],linestyle=[0,0,0,0,0,0,0],$
  ...
  ;create bulge and disc spectra using integrated magnitudes. Convert to Jy
...
;    for j=9,n_elements(bulge_Re)-5,10 do bulge_Re[j]=0.5*(bulge_Re[j-1]+bulge_Re[j+1])
  ...
  if n_comp ge 1100 then oplot,wavelength,(bulge_1D/median(orig_1D[0:100])),color=cgcolor('red');/10000;+90
  oplot,wavelength,(disk_1D/median(orig_1D[0:100])),color=cgcolor('blue');/10000;+60
  oplot,wavelength,(orig_1D/median(orig_1D[0:100]));/10000;+30
  oplot,wavelength,(bestfit_1D/median(orig_1D[0:100])),color=cgcolor('purple');/10000;+30,color=cgcolor('red')
  ;oplot,wavelength,((bulge_1D+disk_1D)/median(orig_1D)),color=cgcolor('orange')
  ;oplot,wavelength,((bulge_1D+disk_1D)-median(bulge_1D+disk_1D))/10+10,color=cgcolor('red')
  oplot,wavelength,(resid_1D/median(orig_1D[0:100])),color=cgcolor('olive');/10000,color=cgcolor('green')
  oplot,wavelength,(resid_sky_1D/median(orig_1D[0:100])),color=cgcolor('dark grey');/10000,color=cgcolor('green')
  ...
  if n_comp eq 1100 then begin
    for j=9,n_elements(disk_mag)-5,10 do bulge_mag[j]=0.5*(bulge_mag[j-1]+bulge_mag[j+1])
    oplot,wavelength,(disk_mag/median(bulge_mag[0:100]+disk_mag[0:100])),color=cgcolor('blue');/10000;+60
    oplot,wavelength,(bulge_mag/median(bulge_mag[0:100]+disk_mag[0:100])),color=cgcolor('red');/10000;+90
    oplot,wavelength,((bulge_mag+disk_mag)/median(bulge_mag[0:100]+disk_mag[0:100])),color=cgcolor('purple');/10000;+90
  endif
  ...
  if keyword_set(GC) then begin
    ...
    for k=n1,n2,1 do begin
      ...
      plot,wavelength,mag_in/median(mag_in[0:100]),yrange=[0,1.8],$
        xrange=[start_wavelength-100,end_wavelength+100],$
    ...
    plot,wavelength,mag_in/median(mag_in[0:100]),yrange=[0.4,1.4],$
      ;      xrange=[start_wavelength-100,5500],$
      xrange=[start_wavelength-100,end_wavelength+100],$
      /xstyle,/ystyle,xthick=3,ythick=3,xcharsize=2,ycharsize=2,$;ytickinterval=30,$
